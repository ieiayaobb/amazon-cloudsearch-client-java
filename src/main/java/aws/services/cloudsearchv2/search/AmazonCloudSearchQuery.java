 ...
 * This class is used to build a query query.
 * 
...
public class AmazonCloudSearchQuery {
     ...
     * 
     * query?q=john&cursor=initial&size=100&return=_no_fields
     * {
     *    "status": {
     *       "rid": "+/Xu5s0oHwojC6o=",
     *       "time-ms": 15
     ...
     * 
     * query?q=john&cursor=VegKzpYYQW9JSVFFRU1UeWwwZERBd09EUTNPRGM9ZA&size=100
     * 
     * Required: No
	 */
	...
	/**
     * Defines an expression that can be used to sort results or specify query or filter criteria. You can also specify an expression as a return field. For more information about defining and using expressions, see Configuring Expressions.
     * 
     * You can define and use multiple expressions in a query request. For example, the following request creates two expressions that are used to sort the results and includes them in the query results:
     * 
     * query?q=(and (term field=genres 'Sci-Fi')(term field=genres 'Comedy'))&q.parser=structured
	...
	/**
     * Retrieves highlights for matches in the specified text or text-array field. Highlight options are specified as a JSON object. If the JSON object is empty, the returned field text is treated as HTML and the first match is highlighted with emphasis tags: <em>query-term</em>.
     * 
     ...
     * 
     * max_phrases—specifies the maximum number of occurrences of the query term(s) you want to highlight. By default, the first occurrence is highlighted.
     * 
     * pre_tag—specifies the string to prepend to an occurrence of a query term. The default for HTML highlights is <em>. The default for text highlights is *.
     * 
     * post_tag—specifies the string to append to an occurrence of a query term. The default for HTML highlights is </em>. The default for text highlights is *.
     * 
	...
	/**
     * Controls whether partial results are returned if one or more index partitions are unavailable. When your query index is partitioned across multiple query instances, by default Amazon CloudSearch only returns results if every partition can be queried. This means that the failure of a single query instance can result in 5xx (internal server) errors. When you specify partial=true. Amazon CloudSearch returns whatever results are available and includes the percentage of documents searched in the query results (percent-searched). This enables you to more gracefully degrade your users' query experience. For example, rather than displaying no results, you could display the partial results and a message indicating that the results might be incomplete due to a temporary system outage.
     * 
     * Default: False
     * 
     * Required: No
     ...
     * 
     * defaultOperator—The default operator used to combine individual terms in the query string. For example: defaultOperator: 'or'. For the dismax parser, you specify a percentage that represents the percentage of terms in the query string (rounded down) that must match, rather than a default operator. A value of 0% is the equivalent to OR, and a value of 100% is equivalent to AND. The percentage must be specified as a value in the range 0-100 followed by the percent (%) symbol. For example, defaultOperator: 50%. Valid values: and, or, a percentage in the range 0%-100% (dismax). Default: and (simple, structured, lucene) or 100 (dismax). Valid for: simple, structured, lucene, and dismax.
     * 
     * fields—An array of the fields to query when no fields are specified in a query. If no fields are specified in a query and this option is not specified, all text and text-array fields are searched. You can specify a weight for each field to control the relative importance of each field when Amazon CloudSearch calculates relevance scores. To specify a field weight, append a caret (^) symbol and the weight to the field name. For example, to boost the importance of the title field over the description field you could specify: fields: ['title^5','description']. Valid values: The name of any configured field and an optional numeric value greater than zero. Default: All text and text-array fields. Valid for: simple, structured, lucene, and dismax.
     * 
     * operators—An array of the operators or special characters you want to disable for the simple query parser. If you disable the and, or, or not operators, the corresponding operators (+, |, -) have no special meaning and are dropped from the query string. Similarly, disabling prefix disables the wildcard operator (*) and disabling phrase disables the ability to query for phrases by enclosing phrases in double quotes. Disabling precedence disables the ability to control order of precedence using parentheses. Disabling near disables the ability to use the ~ operator to perform a sloppy phrase query. Disabling the fuzzy operator disables the ability to use the ~ operator to perform a fuzzy query. escape disables the ability to use a backslash (\) to escape special characters within the query string. Disabling whitespace is an advanced option that prevents the parser from tokenizing on whitespace, which can be useful for Vietnamese. (It prevents Vietnamese words from being split incorrectly.) For example, you could disable all operators other than the phrase operator to support just simple term and phrase queries: operators:['and', 'not', 'or', 'prefix']. Valid values: and, escape, fuzzy, near, not, or, phrase, precedence, prefix, whitespace. Default: All operators and special characters are enabled. Valid for: simple.
     * 
     * phraseFields—An array of the text or text-array fields you want to use for phrase searches. When the terms in the query string appear in close proximity within a field, the field scores higher. You can specify a weight for each field to boost that score. The phraseSlop option controls how much the matches can deviate from the query string and still be boosted. To specify a field weight, append a caret (^) symbol and the weight to the field name. For example, to boost phrase matches in the title field over the abstract field, you could specify: phraseFields:['title^3', 'abstract'] Valid values: The name of any text or text-array field and an optional numeric value greater than zero. Default: No fields. If you don't specify any fields with phraseFields, proximity scoring is disabled even if phraseSlop is specified. Valid for: dismax.
     * 
     * phraseSlop—An integer value that specifies how much matches can deviate from the query phrase and still be boosted according to the weights specified in the phraseFields option. For example, phraseSlop: 2. You must also specify phraseFields to enable proximity scoring. Valid values: positive integers. Default: 0. Valid for: dismax.
     * 
     * explicitPhraseSlop—An integer value that specifies how much a match can deviate from the query phrase when the phrase is enclosed in double quotes in the query string. (Phrases that exceed this proximity distance are not considered a match.) explicitPhraseSlop: 5. Valid values: positive integers. Default: 0. Valid for: dismax.
     * 
     * tieBreaker—When a term in the query string is found in a document's field, a score is calculated for that field based on how common the word is in that field compared to other documents. If the term occurs in multiple fields within a document, by default only the highest scoring field contributes to the document's overall score. You can specify a tieBreaker value to enable the matches in lower-scoring fields to contribute to the document's score. That way, if two documents have the same max field score for a particular term, the score for the document that has matches in more fields will be higher. The formula for calculating the score with a tieBreaker is:
     * 
     ...
     * 
     * simple—perform simple searches of text and text-array fields. By default, the simple query parser searches all text and text-array fields. You can specify which fields to query by with the q.options parameter. If you prefix a query term with a plus sign (+) documents must contain the term to be considered a match. (This is the default, unless you configure the default operator with the q.options parameter.) You can use the - (NOT), | (OR), and * (wildcard) operators to exclude particular terms, find results that match any of the specified terms, or query for a prefix. To query for a phrase rather than individual terms, enclose the phrase in double quotes. For more information, see Searching Your Data with Amazon CloudSearch.
     * 
     * structured—perform advanced searches by combining multiple expressions to define the query criteria. You can also query within particular fields, query for values and ranges of values, and use advanced options such as term boosting, matchall, and near. For more information, see Constructing Compound Queries.
     * 
     * lucene—query using the Apache Lucene query parser syntax. For more information, see Apache Lucene Query Parser Syntax.
     * 
     * dismax—query using the simplified subset of the Apache Lucene query parser syntax defined by the DisMax query parser. For more information, see DisMax Query Parser Syntax.
     * 
     * Type: String
     * 
     * Default: simple
	...
	/**
     * The maximum number of query hits to return.
     * 
     * Default: 10
     * Required: No
	 */
	...
	/**
     * The offset of the first query hit you want to return. You can specify either the start or cursor parameter in a request, they are mutually exclusive. For more information, see Paginating Results.
     * 
